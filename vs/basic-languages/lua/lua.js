...
define(["require", "exports"], function (require, exports) {
    ...
    function getHierarchy(position) {
        var line = position.lineNumber;
        var col = position.column;
        var model = window.editor.getModel();
        var lineContent = model.getLineContent(line).substring(0, col - 1);
        
        // This regex captures the chain of words and delimiters before the cursor
        // It handles cases like "raknet.", "raknet:send", "Game:GETser"
        let match = lineContent.match(/([a-zA-Z_][a-zA-Z0-9_]*(?:\s*[\.:]\s*[a-zA-Z_][a-zA-Z0-9_]*)*)\s*([\.:])\s*[a-zA-Z0-9_]*$/i);
        
        if (match) {
            let chain = match[1];
            let delimiter = match[2];
            let hierarchy = chain.split(/\s*[\.:]\s*/);
            return [hierarchy, delimiter];
        }

        // Handle case where we only have the word before the delimiter and NO word after it
        // Example: "game." (at the very end of the line)
        let simpleMatch = lineContent.match(/([a-zA-Z_][a-zA-Z0-9_]*(?:\s*[\.:]\s*[a-zA-Z_][a-zA-Z0-9_]*)*)\s*([\.:])\s*$/i);
        if (simpleMatch) {
            let chain = simpleMatch[1];
            let delimiter = simpleMatch[2];
            let hierarchy = chain.split(/\s*[\.:]\s*/);
            return [hierarchy, delimiter];
        }
    ...
    function getAutocompleteContainerFromName(luaAutocompletes, name) {
        if (!luaAutocompletes) return;
        
        // Case-insensitive lookup
        const lowerName = name.toLowerCase();
        
        // 1. Check direct map first (the module objects themselves)
        for (const key in luaAutocompletes) {
            if (key.toLowerCase() === lowerName) return luaAutocompletes[key];
        }

        // 2. Search in categorized types (Function, Variable, Module, etc)
        for (const kind in luaAutocompletes) {
            ...
            if (autocompletes) {
                for (const key in autocompletes) {
                    if (key.toLowerCase() === lowerName) return autocompletes[key];
                }
            };
        };
        
        // 3. SPECIAL CASE: "Game" is often typed but we define it as "game"
        if (lowerName === "game") {
            const gameObj = getAutocompleteContainerFromName(luaAutocompletes, "game");
            if (gameObj) return gameObj;
        }
    }

    function getAutocompleteContainerFromHierarchy(luaAutocompletes, hierarchy) {
        ...
        for (let index = 1; index < hierarchy.length; index++) {
            ...
            const children = autocompletes.__children__ || autocompletes;
            const child = getAutocompleteContainerFromName(children, key);
    ...
    function RemoveAutocompletes(scope) {
        const proposals = (typeof GetProposals === "function" && GetProposals())
            || (window.GetProposals && window.GetProposals())
            || window.Proposals
            || [];
    ...
    function AddSnippet(kindName, snippetName, data, scope) {
        // Clone data to avoid corrupting the original autocomplete tree
        let snippet = {
            insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
            kind: monaco.languages.CompletionItemKind[kindName] || monaco.languages.CompletionItemKind.Function,
            __scope__: scope // Attach scope to the snippet, not the original data
        };
        for (const key in data) {
            if (key == "__scope__" || key == "__children__" || key == "__using__" || key == "__requires__") {
                continue;
            }
            let value = data[key];
    ...
    function AddAutocompletes(autocompleteContainer, scope, delimiter) {
        ...
        // Supported Monaco kinds
        const categories = [
            "Field", "Method", "Function", "Variable", "Class", "Interface", "Module", 
            "Property", "Enum", "Keyword", "Snippet", "EnumMember", "Constant", 
            "Operator", "Event", "Unit", "Value"
        ];
        const hasCategories = categories.some(cat => autocompleteContainer[cat]);

        if (hasCategories) {
            for (const kind of categories) {
                const autocompletes = autocompleteContainer[kind];
                if (!autocompletes) continue;
                ...
                for (const name in autocompletes) {
                    ...
                    AddSnippet(kind, name, autocompletes[name], scope);
                };
            }
        } else {
            for (const name in autocompleteContainer) {
                if (name.startsWith("__")) continue;
                AddSnippet("Function", name, autocompleteContainer[name], scope);
            }
        }
    }
    ...
    require(['vs/basic-languages/lua/autocompletes.js'], (module) => {
        module.Load()
            .then((luaAutocompletes) => {
                window.__luaAutocompletesTree = luaAutocompletes;
                let lastAutocompleteContainerScope = "root";
                let isRootPresent = false;

                OnDidChangeCursorPosition((position) => {
                    let [hierarchy, delimiter] = getHierarchy(position);
                    
                    const currentScope = (hierarchy.length > 0 && hierarchy.join(".") + (delimiter || "")) || "root";

                    if (lastAutocompleteContainerScope != currentScope) {
                        // Clear the PREVIOUS scope's proposals
                        ...
                        if (hierarchy.length > 0) {
                            let autocompleteContainer = getAutocompleteContainerFromHierarchy(luaAutocompletes, hierarchy);
                            
                            // If we didn't find the exact hierarchy, try searching for the module globally
                            if (!autocompleteContainer && hierarchy.length === 1) {
                                autocompleteContainer = getAutocompleteContainerFromName(luaAutocompletes, hierarchy[0]);
                            }

                            if (autocompleteContainer) {
                                // We are entering a module scope, so clear root if it was there
                                if (isRootPresent) {
                                    RemoveAutocompletes("root");
                                    isRootPresent = false;
                                }
                                
                                let children = autocompleteContainer.__children__ || autocompleteContainer;
                                
                                // Inheritance logic...
                                if (delimiter) {
                                    const baseInstance = getAutocompleteContainerFromName(luaAutocompletes, "Instance");
                                    if (baseInstance && autocompleteContainer !== baseInstance) {
                                        AddAutocompletes(baseInstance.__children__ || baseInstance, currentScope, delimiter);
                                    }
                                    
                                    const lastName = hierarchy[hierarchy.length-1].toLowerCase();
                                    if (lastName === "game" || lastName === "datamodel") {
                                        const dmServices = getAutocompleteContainerFromName(luaAutocompletes, "services");
                                        if (dmServices) {
                                            AddAutocompletes(dmServices.__children__ || dmServices, currentScope, delimiter);
                                        }
                                    }
                                }
                                
                                AddAutocompletes(children, currentScope, delimiter);
                                return;
                            }
                        }

                        // Fallback to root scope if nothing else matched
                        if (!isRootPresent) {
                            AddAutocompletes(luaAutocompletes, "root");
                            isRootPresent = true;
                        }
                        lastAutocompleteContainerScope = "root";
                    };
                });
