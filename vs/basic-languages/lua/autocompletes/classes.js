		...
		"Class": {
			...
			"RakNetPacket": {
				__children__: {
					"Field": {
						"AsBuffer": {
							label: { name: "AsBuffer", type: "(buffer)" },
							documentation: { value: "The packet payload as a buffer." }
						},
						"AsString": {
							label: { name: "AsString", type: "(string)" },
							documentation: { value: "The packet payload as a string." }
						},
						"AsArray": {
							label: { name: "AsArray", type: "(table)" },
							documentation: { value: "The packet payload as an array of bytes." }
						},
						"Priority": {
							label: { name: "Priority", type: "(number)" },
							documentation: { value: "The RakNet send priority." }
						},
						"Reliability": {
							label: { name: "Reliability", type: "(number)" },
							documentation: { value: "The RakNet reliability mode." }
						},
						"OrderingChannel": {
							label: { name: "OrderingChannel", type: "(number)" },
							documentation: { value: "The channel used for ordered traffic." }
						},
						"Size": {
							label: { name: "Size", type: "(number)" },
							documentation: { value: "The payload size in bytes." }
						},
						"PacketId": {
							label: { name: "PacketId", type: "(number)" },
							documentation: { value: "The ID of the packet." }
						}
					},
					"Function": {
						"SetData()": {
							insertText: "SetData(${1:data})",
							documentation: { value: "```lua\nfunction SetData(data: buffer | string | table)\n```\nReplaces the current packet payload with data." }
						},
						"Block()": {
							insertText: "Block()",
						...
						"Drop()": {
							insertText: "Drop()",
							documentation: { value: "```lua\nfunction Drop()\n```\nPrevents the current packet from being received." }
						}
					}
				}
			}
